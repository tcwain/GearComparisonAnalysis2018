...
urlcolor: blue
vignette: >
  %\VignetteIndexEntry{"Test Size Sel"}
...
Tests of new size-selectivity methods, primarily from Kotwicki et al. (2017, Fish. Res. 191:76-86).
...
We analyze size-frequencies for the same set of species, but do not 
subdivide salmon species by age group. To process the data, eliminate 
records with no size measurements, clean up the list of species names, and
bin them into 5 mm intervals for analysis.

```{r GetSizeData}
```

Next, run the Size-Frequency (SF) analysis for each species. First tabulate the distributions
across all samples by species and gear type. Then, plot the SFs and 
...
```{r SizeSummary, results='hold', eval=FALSE}
```

Then, conduct full gear Size-Selectivity Analysis, as in Kotwicki et al. (2017, Fish. Res. 191:76-86). We use their approach # 1 (their equations 1-4), where effort data is available and comparable between gears, so size distributions are compared on the basis of CPUE. Their equations assume a paired-trawl or split-trawl experimental design, so adjustments must be made for our stratified design.

First, we define functions for each of the methods:
```{r SizeSelMethods, results='hold'}
```

```{r SizeSelAnal, results='hold', warning=FALSE}
```
...
```{r RunStats, fig.width=6, fig.height=3, eval=FALSE}
```
