...
This is an R Markdown document generated using the R
contributed package ``"knitr"``. It can be run from the R command prompt, 
or from the RStudio console to generate a web (HTML) page that includes 
both content as well as the output of any embedded R code chunks
within the document.

**NOTE:** "R" is open source software freely available from 
http://www.R-project.org. "RStudio" is open-source software freely 
available from http://www.rstudio.com. The R contributed packages 
``"rmarkdown"``, ``"knitr"`` and ``"MASS"`` need to be installed before 
running this analysis.

**From the RStudio console**, open the document "MED_Gear_Comparison.Rmd",
and click the "Knit HTML" button. Click the "Help" toolbar button for more 
details on using ``knitr`` and R Markdown.

**From the R command line** in the directory containing the file 
"MED_Gear_Comparison.Rmd", run the command
``rmarkdown::render("MED_Gear_Comparison.Rmd")``. **NOTE** that this method
also requires that the ``pandoc`` libraries are installed on your system.
...
For this, we create an artificial "time" scale for plotting, creating equal 
intervals within blocks. Then, CPUE is computed as counts for each species divided 
by effort.
...
## Part 3: Size-selectivity analysis

```{r file3, echo=FALSE}
knitr::read_chunk(system.file('scripts', 'LenFreqComparison.R',
                        package = "GearComparisonAnalysis2018"))
```
...
We analyze length-frequencies for the same set of species, but do not 
subdivide salmon species by age group. To process the data, eliminate 
records with no length measurements, clean up the list of species names, and
bin them into 5 mm intervals for analysis.

```{r GetLenFreqData}
```

Next, run the LF analysis for each species. First tabulate the distributions
across all samples by species and gear type. Then, plot the LFs and 
apply both a Wilcox-Mann-Whitney test and a Kolmogorov-Smirnov test for 
gear differences. **NOTE** the several warnings that p-values are approximate 
for the K-S test. We'll ignore the warnings, assume they're close enough as
we're concerned with the patterns of size-selectivity, not significance tests.

```{r LenFreqAnal, results='hold'}
```

## Part 4: Catch Ratio Statistics
...
To make the coding easier, we define standard names for the gear types, and 
set the rounding parameters for easy-to-read results tables.
...
_"Modern Applied Statistics with S, 4th ed."_). The ``glm.nb()`` function is 
used to estimate the $\theta$ parameter; if that estimation fails, it reverts
to the value specified in "init.theta". Then (because ``glm.nb()`` fails 
for some species) ``glm()`` is used with the estimated 
$\theta$ to get the final statistics. This means that the distribution of the 
final catch ratio estimate does not inlcude error in the estimation of $\theta$.
...
results for plotting. The first list contains one element for each species,
holding a matrix of summary results for estimates of the ratio _A_ by each 
of the various estimation methods.  Rows are the methods, columns are a 
six-number summary: Mean, Median, and quantiles(0.05, 0.25, 0.75, 0.95).

The routine then loops through the species, computing first the GLM Poisson
catch ratio estimates, then the GLM negative-binomial estimates. The 
Poisson results are used to get a crude initial estimate of the neg-bin 
$\Theta$ parameter, which is used to initiate the refined estimate via 
``glm.nb()``. If the refined estimate fails, the initial estimate of 
$\Theta$ is used instead. 
